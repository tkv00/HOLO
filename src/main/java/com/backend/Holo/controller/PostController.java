...
public class PostController {
    ...
    @PostMapping("/write")
    public ResponseEntity<?> writePost(@Validated @RequestBody PostEntity postEntity, BindingResult bindingResult) {
        ...
        try {
            ...
            logger.info("게시물 등록 성공 - postId: {}, userId: {}", savedPost.getPostId(), savedPost.getUserId());
            return ResponseEntity.ok(savedPost);
        } catch (Exception e) {
            logger.error("게시글 등록 중 오류가 발생했습니다.", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("게시글 등록 중 오류가 발생했습니다. 에러 번호: " + HttpStatus.INTERNAL_SERVER_ERROR.value());
        }
    }
    // 게시물 상세페이지 표시 Api
    @GetMapping("/{postId}")
